...
//   const { addPackToCart, startOrUpdateInProgressPack } = useCart();
...
//   const progressPercent = Math.min((total / pack.minAmount) * 100, 100);
...
//   // Persist in-progress pack whenever quantities/selection changes
//   useEffect(() => {
//     const packData = {
//       packId: pack.id,
//       packName: pack.name,
...
//       items: selectedItems.map((id) => ({ id, quantity: quantities[id] || 0 })),
//       total,
//     };
//     startOrUpdateInProgressPack(packData);

//     // Alert when crossing min threshold and within a small buffer (min -> min+100)
//     if (total > pack.minAmount && total <= pack.minAmount + 100 && !alertedFilled) {
//       toast.success(`${pack.name} filled: ₹${total.toFixed(2)} — You can order now.`);
//       setAlertedFilled(true);
//     }
//     if (total <= pack.minAmount) {
//       setAlertedFilled(false);
//     }
//   }, [selectedItems, quantities, total]);

//   const handleOrderNow = () => {
//     if (total < pack.minAmount) {
//       toast.error(`Please add products worth at least ₹${pack.minAmount}. Current: ₹${total.toFixed(2)}`);
//       return;
//     }

//     const packItems = selectedItems.map((productId) => ({
//       id: productId,
...
//               }}
//             >
//               Select products to reach ₹{pack.minAmount}
//             </p>
//           </div>
//         </div>
...
//             }}
//           >
//             ⚠️ Add ₹{(pack.minAmount - total).toFixed(2)} more to complete this pack
//           </div>
//         )}
...
//             disabled={total < pack.minAmount}
//             style={{
//               padding: '12px 24px',
//               fontFamily: "'Inter', sans-serif",
//               fontSize: '14px',
//               fontWeight: 600,
//               backgroundColor: total >= pack.minAmount ? '#000' : '#ccc',
//               color: '#fff',
//               border: 'none',
//               borderRadius: '8px',
//               cursor: total >= pack.minAmount ? 'pointer' : 'not-allowed',
//               transition: 'all 0.3s ease',
//             }}
//             onMouseEnter={(e) => {
//               if (total >= pack.minAmount) {
